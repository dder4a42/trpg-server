...
export function createApp(config = {}) {
    const app = express();
    const { corsOrigins = ['http://localhost:3000', 'http://127.0.0.1:3000'], trustProxy = false, logFormat = process.env.NODE_ENV === 'production' ? 'combined' : 'dev', } = config;
    // View engine setup
    ...
    if (config.authModule) {
        ...
        const authService = config.authModule['authService'];
