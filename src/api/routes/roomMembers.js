 ...
 */
router.get('/:roomId/members', asyncHandler(async (req, res) => {
...
router.post('/:roomId/join', asyncHandler(async (req, res) => {
    ...
    if (memberCount >= room.state.config.maxPlayers) {
        throw createError('Room is full', 403, 'ROOM_FULL');
    }
    // Check if already member
    const isAlreadyMember = await dbService.roomMemberships.isUserInRoom(roomId, req.user.id);
    if (isAlreadyMember) {
