...
import type { AuthModule } from '@/api/middleware/AuthModule.js';
...
export function setAuthModule(module: AuthModule | null): void {
  authModule = module;
}
...
// Extended room data for lobby display (POJO - Plain Old JavaScript Object)
...
async function requireAuth(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  ...
  if (!session || session.expiresAt < new Date()) {
    if (session) await dbService.userSessions.delete(sessionId);
...
async function getMemberName(room: IRoom, userId: string): Promise<string | undefined> {
...
function isAdminCheck(user?: User): boolean {
  if (authModule) {
 ...
 */
async function renderConversationHistory(history: any[]): Promise<Array<{ role: string; content: string }>> {
...
async function ensureRoom(roomId: string): Promise<IRoom> {
...
function getRoomOr404(roomId: string): IRoom {
...
router.get('/lobby', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  const dbService = DatabaseService.getInstance();
  const activeRooms = dbService.rooms.listRooms(true);
  const roomsForLobby: LobbyRoomData[] = [];
...
router.post('/lobby/create', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
...
router.post('/lobby/join', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
...
router.get('/game', requireAuth, (req: AuthenticatedRequest, res: Response) => {
...
router.get('/game/new', requireAuth, (req: AuthenticatedRequest, res: Response) => {
  // Generate a new room ID and redirect
  const roomId = generateRoomId();
  res.redirect(`/game/${roomId}`);
});
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    // Load more history for initial page render (50 messages)
    const history = room.getConversationHistory().getRecent(50);
    const gameState = room.getGameState();
    const worldContext = gameState.worldContext;
    const members = await room.getMembers();
    const nameByCharacterId = new Map(
      members
        .filter((member) => member.characterId)
        .map((member) => [
          member.characterId as string,
          member.characterName || member.username,
        ])
    );
    const characterConditions = Array.from(gameState.characterOverlays.values()).map((overlay) => ({
      characterId: overlay.characterId,
      characterName: nameByCharacterId.get(overlay.characterId) || overlay.characterId,
...
router.get('/characters', requireAuth, (req: AuthenticatedRequest, res: Response) => {
...
router.get('/characters/create', requireAuth, (req: AuthenticatedRequest, res: Response) => {
...
export function getRoomsMap(): Map<string, IRoom> {
  return rooms;
}
