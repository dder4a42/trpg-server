...
// Extend Express Request type (auth handled at app level)
interface AuthenticatedRequest extends Request {
  user?: any;
  sessionId?: string;
}

type ReadyRoomMemberVM = {
  id: number;
  userId: string;
  username: string;
  characterId?: string;
...
async function buildReadyRoomState(req: AuthenticatedRequest, roomId: string): Promise<ReadyRoomStateVM> {
  ...
  if (!userMembership) {
    const err: any = new Error('Not in room');
    err.statusCode = 403;
  ...
  const enrichedMembers: ReadyRoomMemberVM[] = await Promise.all(
...
async function renderReadyRoom(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    const code = e?.code;
    if (code === 'ROOM_FORBIDDEN') return res.redirect('/lobby?error=ROOM_FORBIDDEN');
    if (code === 'ROOM_NOT_FOUND') return res.redirect('/lobby?error=ROOM_NOT_FOUND');
    if (code === 'NOT_IN_ROOM') return res.redirect('/lobby');
    if (code === 'ROOM_SUSPENDED') return res.redirect('/lobby?error=ROOM_SUSPENDED');
    throw e;
  }
}
...
router.get('/:roomId/state',  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { roomId } = req.params;
  const db = DatabaseService.getInstance();
  const roomRecord = db.rooms.getRoomById(roomId);
  if (roomRecord && (roomRecord.lifecycleState === 'IN_GAME' || roomRecord.gameStarted)) {
...
router.post('/:roomId/module',  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { roomId } = req.params;
  ...
  const room = db.rooms.getRoomById(roomId);
  if (!room) return res.status(404).json({ success: false, error: { code: 'ROOM_NOT_FOUND' } });
...
router.post('/:roomId/select-character',  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { roomId } = req.params;
...
router.post('/:roomId/ready',  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { roomId } = req.params;
  const db = DatabaseService.getInstance();
...
router.post('/:roomId/start',  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { roomId } = req.params;
  const db = DatabaseService.getInstance();
...
router.post('/:roomId/leave',  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { roomId } = req.params;
  const db = DatabaseService.getInstance();
...
router.post('/:roomId/chat',  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { roomId } = req.params;
...
router.post('/:roomId/end',  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { roomId } = req.params;
  const db = DatabaseService.getInstance();
...
router.get('/:roomId/status',  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { roomId } = req.params;
  const db = DatabaseService.getInstance();
