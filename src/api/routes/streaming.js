...
router.get('/rooms/:roomId/stream', asyncHandler(async (req, res) => {
...
router.get('/rooms/:roomId/ws', (req, res) => {
...
export function broadcastToRoom(roomId, event, data) {
    ...
    // console.log(`[SSE] Broadcasting to ${clients.length} clients in room ${roomId}, event: ${event}`);
 ...
 */
export async function* streamLLMToRoom(roomId, input) {
    const room = getRooms().get(roomId);
    if (!room) {
 ...
 */
export async function* streamCombinedLLMToRoom(roomId) {
    const room = getRooms().get(roomId);
    if (!room) {
        throw new Error(`Room ${roomId} not found`);
    }
 ...
 */
export function broadcastChatMessage(roomId, message) {
