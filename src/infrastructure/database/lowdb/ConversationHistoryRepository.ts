...
export class ConversationHistoryRepository {
   ...
   */
  private getNextTurnNumber(roomId: string): number {
   ...
   */
  getHistory(roomId: string, limit = 50, offset = 0): ConversationTurn[] {
    const turns = this.db
      .getData()
   ...
   */
  getAllTurns(roomId: string): ConversationTurn[] {
    return this.db
      .getData()
      .conversationTurns.filter((t) => t.room_id === roomId)
      .sort((a, b) => a.turn_number - b.turn_number)
   ...
   */
  getTurnById(turnId: number): ConversationTurn | null {
    const turn = this.db
      .getData()
   ...
   */
  getRecentTurns(roomId: string, count = 10): ConversationTurn[] {
    const turns = this.db
      .getData()
      .conversationTurns.filter((t) => t.room_id === roomId)
   ...
   */
  countTurns(roomId: string): number {
   ...
   */
  async deleteAllTurns(roomId: string): Promise<number> {
    const data = this.db.getData();
